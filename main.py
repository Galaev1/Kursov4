from classes import JSONFileInterface, HH, HHVacancy, SJ, SJVacancy
from init import user_input_for_request, add_json, user_menu_loaded, create_collection_from_file, \
    filter_by_service, filter_by_salary, filter_by_requirement, print_result
...
def main():
    ...
    user_answer = user_menu_loaded()  # (фильтр\ по сервису 1-НН 2-SJ 0-оба, фильтр по зп 0-нет или число)
    if user_answer == 'stop':
        return 0
    create_collection_from_file(file_interface.datafile)  # читаем файл и создаем коллекцию экземпляров
    if user_answer[0]:
        filter_by_service(user_answer[0])  # фильтруем по сервису
    if user_answer[1]:
        filter_by_salary(user_answer[1])  # фильтруем по зп
    if user_answer[2]:
