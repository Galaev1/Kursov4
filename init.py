from classes import Vacancy, HHVacancy, SJVacancy, HH, SJ, JSONFileInterface
...
def user_menu_loaded() -> tuple[int, int, str | int] | str:
    ...
    while True:
        service = input(
            '\t- Фильтровать вакансии по сервису ("1" - только Head Hunter'
            ' | "2" - только Super Job | "0" - оба | "stop" - выход):\n')
        if service in ('0', '1', '2', 'stop'):
            break
    if service == 'stop':
        return 'stop'
